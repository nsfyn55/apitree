...
from pyramid_apitree.api_documentation import APIDocumentationView
...
class TestPrepareItem(unittest.TestCase):
    ...
    def test_custom_class(self):
        ...
        assert self.apidoc_view.prepare_item(CustomClass) == 'CustomClass'
    
    def test_dict_obj(self):
        expected = prep_json({'a': 'object'})
        result = self.apidoc_view.prepare_item({'a': object})
        assert result == expected
    
    def test_list_obj(self):
        expected = prep_json(['object'])
        result = self.apidoc_view.prepare_item([object])
        assert result == expected
    ...
    def test_listof_object(self):
        assert self.apidoc_view.prepare_item(ListOf(object)) == 'ListOf(object)'
    ...
    def test_listof_list(self):
        ...
        result = self.apidoc_view.prepare_item(ListOf([object]))
        assert result == expected
    ...
    def test_listof_dict(self):
        ...
        result = self.apidoc_view.prepare_item(ListOf({'a': object}))
        assert result == expected
